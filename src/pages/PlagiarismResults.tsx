      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            View similarity scores and matched sources for your submitted documents
          </p>
        </div>
      ...
      <div className="mb-6 flex flex-wrap gap-3">
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm">
          ...
          <span className="ml-2 text-emerald-600">
            {results.filter((r) => r.plagiarismPercent <= 20).length}
          </span>
        </div>
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-sm">
          ...
          <span className="ml-2 text-amber-600">
            {results.filter((r) => r.plagiarismPercent > 20 && r.plagiarismPercent <= 49).length}
          </span>
        </div>
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm">
          ...
          <span className="ml-2 text-red-600">
            {results.filter((r) => r.plagiarismPercent >= 50).length}
          </span>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {results.length === 0 ? (
            <p className="p-12 text-center text-sm text-muted-foreground">
              No plagiarism results yet. Upload a document to get started.
            </p>
          ) : (
                ...
                <TableBody>
                  {results.map((result) => (
                    <TableRow key={result.id}>
                      ...
                      <TableCell>
                        <PlagiarismBadge percent={result.plagiarismPercent} showPercent={false} />
                      </TableCell>
                      <TableCell className="text-muted-foreground">{result.matchedSections.length}</TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="sm" onClick={() => setViewScan(result)}>
                            <Eye className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => downloadScanReport(result)}>
          ...
          {viewScan && (
            <>
              ...
              <div className="space-y-4 text-sm">
                ...
                <p>
                  <strong>Original content:</strong> {viewScan.originalPercent}%
                </p>
                <p>
                  <strong>Word count:</strong> {viewScan.wordCount.toLocaleString()}
                </p>
